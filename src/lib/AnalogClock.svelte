<script lang="ts">
  ...
  setInterval(() => {
    date = new Date();
    seconds = date.getSeconds() * degree + base;
    minutes = date.getMinutes() * degree + base;
    hours = date.getHours() * 30 + date.getMinutes() * 0.5 + base;
  }, 1000);
  ...
  const nailStyle =
    "transition absolute duration-1000 w-2 h-[50%] origin-bottom";
...
{#if hours && minutes && seconds}
  ...
  >
    ...
    >
      ...
      >
        <i
          class="bg-red-600 w-1 {nailStyle} before:bg-red-600 before:-left-1 {nailBeforeStyle}"
      ...
      >
        <i
          class="bg-indigo-600 w-2 {nailStyle} before:bg-indigo-600 before:-left-1 {nailBeforeStyle}"
      ...
      >
        <i
          class="bg-lime-600 w-3 {nailStyle} before:bg-lime-600 before:-left-[1px] {nailBeforeStyle}"
